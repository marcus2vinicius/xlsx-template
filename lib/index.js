...
module.exports = (function () {
    ...
    Workbook.prototype.substitute = function (sheetName, substitutions) {
        ...
        var sheetData = sheet.root.find("sheetData"),
            currentRow = null,
            totalRowsInserted = 0;
        var rels = self.loadSheetRels(sheet.root, sheet.filename);

        if (rels === null) {

            throw Error("Rels not found in sheet: " + sheetName);
        }
        ;

        var namedTables = self.loadTables(sheet.root, sheet.filename, rels.root),
            rows = [];
        var drawing = null;
        sheetData.findall("row").forEach(function (row) {
            ...
            row.findall("c").forEach(function (cell) {
                ...
                if (cell.attrib.t === "s") {
                    ...
                    self.extractPlaceholders(string).forEach(function (placeholder, idx) {

                        // Only substitute things for which we have a substitution
                        var substitution = placeholder.name.split('.').reduce(function (prev, curr) {
                        ...
                        if (placeholder.type === "table" && substitution instanceof Array) {
                            newCellsInserted = self.substituteTable(
                                row, newTableRows,
                                cells, cell,
                                namedTables, substitution, placeholder.key, placeholder
                            );
    ...
    Workbook.prototype.loadSheetRels = function (sheet, sheetFilename) {
    ...
    Workbook.prototype.loadTables = function (sheet, sheetFilename, rels) {
        var self = this;
    ...
    Workbook.prototype.loadDrawing = function (sheet, sheetFilename, rels) {
        ...
        var sheetDirectory = path.dirname(sheetFilename),
            sheetName = path.basename(sheetFilename),
            drawing = {filename: '', root: null}; // [{filename: ..., root: ....}]


        var drawingPart = sheet.find("drawing");
        if (drawingPart === null) {

            drawing = self.initDrawing(sheet, sheetFilename, rels);
    ...
    Workbook.prototype.initDrawing = function (sheet, sheetFilename, rels) {

        var self = this;
        var maxId = self.findMaxId(rels, 'Relationship', 'Id', /rId(\d*)/);

        //var relationships = rels.find('Relationships');
        var rel = etree.SubElement(rels, 'Relationship');

        //etree.SubElement(sheet, 'drawing', {'r:id':'rId'+maxId});

        sheet.insert(sheet.getchildren().indexOf(sheet.find('tableParts')), etree.Element('drawing', {'r:id':'rId'+maxId}));
    ...
    Workbook.prototype.insertCellValue = function (cell, substitution, placeholder) {
        var self = this;
        ...
        if (typeof(substitution) === "number") {
            ...
            cell.attrib.t = "s";
            if (placeholder && !placeholder.full) {
                var oldValue = self.sharedStrings[Number(cellValue.text)];
                stringified = oldValue.replace(placeholder.placeholder, self.stringify(stringified));
            }
    ...
    Workbook.prototype.substituteImage = function (cell, string, placeholder, substitution, drawing) {
        ...
        var imagePart = etree.SubElement(drawing.root, 'xdr:twoCellAnchor', {'editAs': 'oneCell'});
        ...
        var toPart = etree.SubElement(imagePart, 'xdr:to');
        var toCol = etree.SubElement(toPart, 'xdr:col');
        toCol.text = (Number.parseInt(self.charToNum(self.splitRef(cell.attrib.r).col))).toString();
        var toColOff = etree.SubElement(toPart, 'xdr:colOff');
        toColOff.text = (Number.parseInt(self.pixelsToEMUs(dimension.width))+172900).toString();

        var toRow = etree.SubElement(toPart, 'xdr:row');
        toRow.text = (Number.parseInt(self.splitRef(cell.attrib.r).row)+6).toString();
        var toRowOff = etree.SubElement(toPart, 'xdr:rowOff');
        toRowOff.text = Number.parseInt(self.pixelsToEMUs(dimension.height)).toString();
        ...
        //var dimensionPart = etree.SubElement(imagePart, 'xdr:ext', {
        //    'cx': self.pixelsToEMUs(dimension.width),
        //    'cy': self.pixelsToEMUs(dimension.height)
        //});

        var picNode = etree.SubElement(imagePart, 'xdr:pic');
        var nvPicPr = etree.SubElement(picNode, 'xdr:nvPicPr');
        var cNvPr = etree.SubElement(nvPicPr, 'xdr:cNvPr', {id: maxId, name: '??' + maxId, descr: ''});
        ...
        var extLst = etree.SubElement(blip, 'a:extLst');
        var ext = etree.SubElement(extLst, 'a:ext', {'uri': "{28A0092B-C50C-407E-A947-70E740481C1C}"});
        var useLocalDpi = etree.SubElement(ext, 'a14:useLocalDpi', {'xmlns:a14': "http://schemas.microsoft.com/office/drawing/2010/main", 'val': "0"});
        var stretch = etree.SubElement(blipFill, 'a:stretch');
        var fillRect = etree.SubElement(stretch, 'a:fillRect');
        var spPr = etree.SubElement(picNode, 'xdr:spPr');  //, {bwMode: 'auto'}
        var xfrm = etree.SubElement(spPr, 'a:xfrm');
        var off = etree.SubElement(xfrm, 'a:off', {x:"0", y:"0" });
        ext = etree.SubElement(xfrm, 'a:ext', { cx:self.pixelsToEMUs(dimension.width), cy:self.pixelsToEMUs(dimension.height) });//{cx:1800000, cy:2752381})
    ...
    Workbook.prototype.findMaxId = function (element, tag, attr, idRegex) {
        ...
        var maxEle = element.findall(tag).reduce(function (p, c) {
            var pid = 0;
            var cid = 0;
            ...
            var match = idRegex.exec(c.attrib[attr]);
            if (match == null) {
                throw new Error("Can not find the id!");
            }
            cid = match[1];

            match = idRegex.exec(p.attrib[attr]);
            if (match == null) {
                throw new Error("Can not find the id!");
            }
            pid = match[1];
        ...
        if(maxEle!=null){
            ...
            maxId = idRegex.exec(maxEle.attrib[attr])[1];
    ...
    Workbook.prototype.substituteTable = function (row, newTableRows, cells, cell, namedTables, substitution, key, placeholder) {
        var self = this,
            originalCell = self.cloneElement(cell),
        ...
        if (substitution.length === 0) {
            ...
            substitution.forEach(function (element, idx) {
                var newRow, newCell,
                    ...
                    newCells = [],
                    value = key.split('.').reduce(function (prev, curr) {
                            return prev ? prev[curr] : undefined;
                        }, element) || '';

                if (idx === 0 || placeholder.multiple) { // insert in the row where the placeholders are

                    if (idx > 0 && placeholder.multiple) {
                        newRow = newTableRows[idx - 1];
                        cell = newRow.findall('c').pop();
                    }
                    ...
                    if (value instanceof Array) {
                        ...
                        self.insertCellValue(cell, value, placeholder);
                    }
                    ...
                    // Create a new cell
                    newCell = self.cloneElement(originalCell);
                    newCell.attrib.r = self.joinRef({
                        row: newRow.attrib.r,
